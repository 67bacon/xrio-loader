...
local WORKER_URL = "https://xrio-loader.houm7755.workers.dev"  -- ← REPLACE with your deployed Worker URL
...
-- Call Worker /loader with {key, hwid}. Returns script source or nil + error msg.
local function fetchScript(key)
    local body = HttpService:JSONEncode({ key = key, hwid = hwid() })
    local ok, res = pcall(function()
        return httpRequest({
            Url = WORKER_URL .. "/loader",
            Method = "POST",
            Headers = { ["Content-Type"] = "application/json" },
            Body = body,
        })
    end)
    if not ok or type(res) ~= "table" then return nil, "network error" end
    if res.StatusCode == 200 and res.Body then return res.Body, nil end
    -- Worker returns JSON error on auth fail
